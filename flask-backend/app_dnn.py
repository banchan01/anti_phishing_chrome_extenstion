...
import os
import joblib  # Required for saving and loading the scaler
...
# Load the DNN model
model = load_model("../ml/model/DNN_model.h5")

# Load the scaler
scaler = joblib.load("../ml/model/DNN_scaler.joblib")  # Load the scaler saved with joblib
...
def check_url():
    try:
        ...
        # Make prediction with ordered features and apply scaling
        feature_array = np.array([[features[name] for name in FEATURE_NAMES]], dtype=np.float32)
        scaled_features = scaler.transform(feature_array)  # Apply scaling
